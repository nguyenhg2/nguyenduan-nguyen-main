...
const STATUS_CHIP: Record<string, { label: string; color: 'error' | 'warning' | 'info' | 'success' | 'default' }> = {
  OPEN:        { label: 'Moi bao cao', color: 'error' },
  DISPATCHED:  { label: 'Da dieu phoi', color: 'warning' },
  IN_PROGRESS: { label: 'Dang xu ly', color: 'info' },
  RESOLVED:    { label: 'Hoan thanh', color: 'success' },
}
...
export default function UnitPortal({ unit, companyId, onClose }: Props) {
  ...
  const load = async () => {
    if (!unitId) return
    try {
      const [incRes, compRes, typeRes] = await Promise.all([
        api.get<Incident[]>(`/api/incidents?unitId=${unitId}`),
        api.get<Component[]>(`/api/components?unitId=${unitId}`),
        api.get<IncidentType[]>(`/api/incident-types?companyId=${companyId}`),
      ])
  ...
  // Priority tu dong lay tu IncidentType.defaultPriority
  ...
  const handleDeleteComp = async (comp: Component) => {
    ...
    if (!realId) return
    if (!window.confirm('Xac nhan xoa bien che nay?')) return
  ...
  const compDialog = ReactDOM.createPortal(
    <Dialog open={openCompDialog} onClose={handleCloseCompDialog} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        <Stack spacing={2} sx={{ pt: 1 }}>
          ...
          <Stack direction="row" spacing={2}>
            <TextField
              label="Ten bien che" required fullWidth size="small"
              value={editingComp.name ?? ''}
              onChange={e => setEditingComp(p => ({ ...p, name: e.target.value }))}
            />
            <TextField
              label="Loai thiet bi" fullWidth size="small"
          ...
          <FormControl fullWidth size="small">
            ...
            >
              ...
              <MenuItem value="MAINTENANCE">Dang bao tri</MenuItem>
            </Select>
          </FormControl>

          <Divider textAlign="left">
            <Typography variant="caption" color="text.secondary">Thong tin mang</Typography>
          </Divider>

          <Stack direction="row" spacing={2}>
            <TextField label="Dia chi IP" fullWidth size="small"
              value={editingComp.ipAddress ?? ''}
              onChange={e => setEditingComp(p => ({ ...p, ipAddress: e.target.value }))} />
            <TextField label="Dia chi MAC" fullWidth size="small"
          ...
          <Divider textAlign="left">
            <Typography variant="caption" color="text.secondary">Thong tin phan cung</Typography>
          </Divider>

          <Stack direction="row" spacing={2}>
            <TextField label="Nha san xuat" fullWidth size="small"
          ...
          <Stack direction="row" spacing={2}>
            <TextField label="He dieu hanh (OS)" fullWidth size="small"
          ...
          <Divider textAlign="left">
            <Typography variant="caption" color="text.secondary">Khac</Typography>
          </Divider>

          <TextField label="Vi tri vat ly" fullWidth size="small"
            value={editingComp.location ?? ''}
            onChange={e => setEditingComp(p => ({ ...p, location: e.target.value }))} />
          <TextField label="Ghi chu" fullWidth size="small" multiline rows={2}
      ...
      <DialogActions>
        <Button onClick={handleCloseCompDialog}>Huy</Button>
        <Button variant="contained" onClick={handleSubmitComp} disabled={!editingComp.name}>
          {(editingComp as any)._id ? 'Cap nhat' : 'Them moi'}
        </Button>
      </DialogActions>
    </Dialog>,
    document.body
    ...
    <>
      <Box sx={{
        ...
        <Paper elevation={3} square sx={{
          ...
          {activeInc.length > 0 && (
            <Chip label={`${activeInc.length} su co dang mo`} color="error" size="small" />
          )}
          {unit.remoteAccessReady && (
            <Chip label="Remote Ready" color="info" size="small" />
          )}
          ...
          {loading ? (
            ...
            <>
                ...
                <Stack spacing={3} maxWidth={700}>
                  <Paper variant="outlined" sx={{ p: 2.5 }}>
                    ...
                    <Stack spacing={2}>
                      ...
                      {autoPriority !== null && (
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            (xac dinh theo loai su co)
                          </Typography>
                        </Stack>
                      )}

                      <FormControl fullWidth size="small">
                        <InputLabel>Bien che lien quan (tuy chon)</InputLabel>
                        <Select
                          value={selectedCompId}
                          label="Bien che lien quan (tuy chon)"
                      ...
                      >
                        {submitting ? 'Dang gui...' : 'Gui bao cao su co'}
                      </Button>
                    </Stack>
                  </Paper>
                  ...
                  {activeInc.length > 0 && (
                    <Paper variant="outlined" sx={{ p: 2 }}>
                      <Typography variant="subtitle1" fontWeight="bold" gutterBottom>
                        Su co dang xu ly ({activeInc.length})
                      </Typography>
                      <Stack spacing={1}>
                        {activeInc.map(inc => (
                          <Paper key={inc._id} variant="outlined" sx={{ p: 1.5 }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={1}>
                              ...
                              {(inc.status === 'OPEN' || inc.status === 'IN_PROGRESS') && (
                                ...
                                >
                                  {resolvingId === inc._id ? '...' : 'Da xu ly xong'}
                                </Button>
                              )}
                            </Stack>
                          </Paper>
                  ...
                  {activeInc.length === 0 && (
                    <Alert severity="success">Don vi khong co su co dang xu ly.</Alert>
                  )}
                </Stack>
              )}
                ...
                <Stack spacing={2}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Typography variant="subtitle1" fontWeight="bold">
                      Danh sach bien che ({components.length})
                    </Typography>
                  ...
                  {components.length === 0 ? (
                    <Alert severity="info">
                      Chua co bien che nao. Nhan "Them bien che" de them moi.
                    </Alert>
                  ) : (
                    <Table size="small">
                      <TableHead>
                        ...
                        {components.map(c => (
                          <TableRow key={getId(c)}>
                            ...
                            <TableCell>
                              <Chip
                                label={
                                  c.status === 'ACTIVE' ? 'Hoat dong'
                                  : c.status === 'INACTIVE' ? 'Ngung'
                                  : 'Bao tri'
                                }
                                color={
                            ...
                            <TableCell align="right">
                              <Stack direction="row" justifyContent="flex-end" gap={0.5}>
                                <Button size="small" onClick={() => handleOpenEditComp(c)}>Sua</Button>
                                <Button size="small" color="error" onClick={() => handleDeleteComp(c)}>Xoa</Button>
                              </Stack>
                            </TableCell>
                          </TableRow>
                        ))}
              ...
              {tab === 2 && (
                <Stack spacing={1}>
                  <Typography variant="subtitle1" fontWeight="bold" gutterBottom>
                    Lich su su co ({resolvedInc.length})
                  </Typography>
                  {resolvedInc.length === 0 ? (
                    <Alert severity="info">Chua co su co nao da hoan thanh.</Alert>
                  ) : (
                    resolvedInc.map(inc => (
                      <Paper key={inc._id} variant="outlined" sx={{ p: 1.5 }}>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={1}>
                          <Stack spacing={0.5}>
                            ...
                            />
                            <Typography variant="caption" color="text.secondary">
                              Bao cao: {new Date(inc.reportedAt).toLocaleString('vi-VN')}
                            </Typography>
                            {inc.resolvedAt && (
                              <Typography variant="caption" color="text.secondary">
                                Hoan thanh: {new Date(inc.resolvedAt).toLocaleString('vi-VN')}
                              </Typography>
                            )}
                          </Stack>
                          <Chip label="Hoan thanh" color="success" size="small" />
                        </Stack>
                      </Paper>
                    ))
                  )}
